import { SearchIcon, FilterIcon, SortAscIcon, TagIcon, DatabaseIcon, DownloadIcon } from "lucide-react";
...
export function DatasetsPage() {
  ...
  const datasets = [{
    id: 1,
    name: "Tunisian COVID-19 X-Ray Dataset",
    creator: "CHU Tunis",
    description: "Collection de radiographies pulmonaires annotées de patients COVID-19",
    size: "15GB",
    downloads: "2.3K",
    type: "Medical Images",
    language: "N/A",
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        <div className="flex-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {datasets.map((dataset, index) => <div key={index} className="bg-white rounded-lg shadow hover:shadow-md transition-shadow">
              <div className="p-6">
                <div className="flex items-center mb-4">
                  ...
                  <div>
                    <Link to={"/datasets/1"} className="text-xl font-semibold text-gray-900">
                      {dataset.name}
                    </Link>
                    <Link to={`/creator/${dataset.creator}`} className="text-sm text-gray-500">
                      {dataset.creator}
                    </Link>
                  </div>
                </div>
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                  ...
                  {dataset.language !== "N/A" && <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800">
                    {dataset.language}
                  </span>}
                </div>
